...
#toolbar{position:absolute;top:4px;right:10px;z-index:100;display:flex;gap:4px;padding:4px;background:rgba(30,34,45,0.9);border-radius:4px}
.tool-btn{padding:4px 10px;font-size:11px;font-family:Consolas,monospace;background:#2B2B43;color:#d1d4dc;border:1px solid #444;border-radius:3px;cursor:pointer}
.tool-btn:hover{background:#3B3B53}
.tool-btn.active{background:#4B7BE5;color:#fff;border-color:#4B7BE5}
.tool-btn:disabled{opacity:0.4;cursor:not-allowed}
#textInput{position:absolute;z-index:101;background:#2B2B43;color:#fff;border:1px solid #4B7BE5;border-radius:3px;padding:4px 8px;font-size:12px;font-family:Consolas,monospace;display:none}
</style>
</head><body>
<div id="wrap">
  ...
  <div id="profile-wrap">
    <div id="toolbar">
      <button class="tool-btn" id="btnSelect" title="Select/Move (V)">Select</button>
      <button class="tool-btn" id="btnHLine" title="Horizontal Line (H)">H-Line</button>
      <button class="tool-btn" id="btnVLine" title="Vertical Line (V)">V-Line</button>
      <button class="tool-btn" id="btnTrend" title="Trend Line (T)">Trend</button>
      <button class="tool-btn" id="btnText" title="Text (X)">Text</button>
      <button class="tool-btn" id="btnDelete" title="Delete Selected (Del)">Delete</button>
      <button class="tool-btn" id="btnClear" title="Clear All (Ctrl+Del)">Clear All</button>
    </div>
    <input type="text" id="textInput" placeholder="Enter text..." />
    <canvas id="profile"></canvas>
  </div>
</div>
<div id="loading" style="display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:9999;background:rgba(19,23,34,0.92);">
  <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#ffffff;font-family:Consolas,monospace;text-align:center">
    <div style="font-size:40px;margin-bottom:16px">&#8987;</div>
    <div style="font-size:24px;font-weight:bold;margin-bottom:8px">Fetching data from Dhan API...</div>
    <div style="font-size:18px;color:#aaa">Please wait...</div>
  </div>
</div>
...
// ===================== DRAWING TOOLS =====================
let _drawMode='select'; // 'select','hline','vline','trend','text'
let _drawnObjects=[]; // {type, x1,y1,x2,y2,text,color,id}
let _tempObj=null; // temporary object being drawn
let _selectedObj=null;
let _dragStart={x:0,y:0};
const _colors=['#ff6b6b','#4ecdc4','#ffe66d','#95e1d3','#f38181','#aa96da','#fcbad3'];

// Tool buttons
document.getElementById('btnSelect').addEventListener('click',()=>setDrawMode('select'));
document.getElementById('btnHLine').addEventListener('click',()=>setDrawMode('hline'));
document.getElementById('btnVLine').addEventListener('click',()=>setDrawMode('vline'));
document.getElementById('btnTrend').addEventListener('click',()=>setDrawMode('trend'));
...
function setDrawMode(mode){
    ...
    if(mode==='select'){document.getElementById('btnSelect').classList.add('active');_selectedObj=null;PR.render();}
    else if(mode==='hline')document.getElementById('btnHLine').classList.add('active');
    else if(mode==='vline')document.getElementById('btnVLine').classList.add('active');
    else if(mode==='trend')document.getElementById('btnTrend').classList.add('active');
    else if(mode==='text')document.getElementById('btnText').classList.add('active');
    document.getElementById('textInput').style.display='none';
}

function deleteSelected(){
    if(_selectedObj!==null){
        _drawnObjects.splice(_selectedObj,1);
        _selectedObj=null;
        PR.render();
    }
}
...
// Override PR.render to include drawn objects
const _origRender=PR.render.bind(PR);
PR.render=function(){
    _origRender();
    drawUserObjects();
};
...
function drawUserObjects(){
    if(!PR.canvas)return;
    const x=PR.canvas.getContext('2d');
    const W=PR.canvas.clientWidth,H=PR.canvas.clientHeight;
    if(W<1||H<1)return;

    x.save();
    x.beginPath();
    x.rect(PR.ml,PR.mt,W-PR.ml-PR.mr,H-PR.mt-PR.mb);
    x.clip();

    // Draw saved objects
    _drawnObjects.forEach((obj,i)=>{
        const sel=(i===_selectedObj);
        const col=obj.color||_colors[i%_colors.length];
        if(obj.type==='hline'){
            const py=PR.py(obj.y1);
            if(py>=PR.mt&&py<=H-PR.mb){
                x.strokeStyle=sel?'#fff':col;x.lineWidth=sel?2:1.5;
                x.setLineDash([5,3]);
                x.beginPath();x.moveTo(PR.ml,py);x.lineTo(W-PR.mr,py);x.stroke();
                x.setLineDash([]);
                if(sel){
                    x.fillStyle=col;
                    x.beginPath();x.arc(PR.ml+10,py,5,0,Math.PI*2);x.fill();
                    x.beginPath();x.arc(W-PR.mr-10,py,5,0,Math.PI*2);x.fill();
                }
            }
        }else if(obj.type==='vline'){
            const px=PR.px(obj.x1);
            if(px>=PR.ml&&px<=W-PR.mr){
                x.strokeStyle=sel?'#fff':col;x.lineWidth=sel?2:1.5;
                x.setLineDash([5,3]);
                x.beginPath();x.moveTo(px,PR.mt);x.lineTo(px,H-PR.mb);x.stroke();
                x.setLineDash([]);
                if(sel){
                    x.fillStyle=col;
                    x.beginPath();x.arc(px,PR.mt+10,5,0,Math.PI*2);x.fill();
                    x.beginPath();x.arc(px,H-PR.mb-10,5,0,Math.PI*2);x.fill();
                }
            }
        }else if(obj.type==='trend'){
            const px1=PR.px(obj.x1),py1=PR.py(obj.y1);
            const px2=PR.px(obj.x2),py2=PR.py(obj.y2);
            x.strokeStyle=sel?'#fff':col;x.lineWidth=sel?2:1.5;
            x.beginPath();x.moveTo(px1,py1);x.lineTo(px2,py2);x.stroke();
            // End points
            x.fillStyle=col;
            x.beginPath();x.arc(px1,py1,4,0,Math.PI*2);x.fill();
            x.beginPath();x.arc(px2,py2,4,0,Math.PI*2);x.fill();
        }else if(obj.type==='text'&&obj.text){
            const px=PR.px(obj.x1),py=PR.py(obj.y1);
            if(px>=PR.ml&&px<=W-PR.mr&&py>=PR.mt&&py<=H-PR.mb){
                x.font='12px Consolas,monospace';
                const tw=x.measureText(obj.text).width;
                const bgH=18,bgW=tw+8;
                x.fillStyle='rgba(30,34,45,0.85)';
                x.fillRect(px-4,py-12,bgW,bgH);
                x.strokeStyle=sel?col:'#555';x.lineWidth=1;
                x.strokeRect(px-4,py-12,bgW,bgH);
                x.fillStyle=sel?'#fff':col;
                x.fillText(obj.text,px,py);
            }
        }
    });

    // Draw temp object (while dragging)
    if(_tempObj){
        const col='#4B7BE5';
        if(_tempObj.type==='hline'){
            const py=PR.py(_tempObj.y1);
            if(py>=PR.mt&&py<=H-PR.mb){
                x.strokeStyle=col;x.lineWidth=1.5;x.setLineDash([5,3]);
                x.beginPath();x.moveTo(PR.ml,py);x.lineTo(W-PR.mr,py);x.stroke();
                x.setLineDash([]);
            }
        }else if(_tempObj.type==='vline'){
            const px=PR.px(_tempObj.x1);
            if(px>=PR.ml&&px<=W-PR.mr){
                x.strokeStyle=col;x.lineWidth=1.5;x.setLineDash([5,3]);
                x.beginPath();x.moveTo(px,PR.mt);x.lineTo(px,H-PR.mb);x.stroke();
                x.setLineDash([]);
            }
        }else if(_tempObj.type==='trend'){
            const px1=PR.px(_tempObj.x1),py1=PR.py(_tempObj.y1);
            const px2=PR.px(_tempObj.x2),py2=PR.py(_tempObj.y2);
...
// Track drawing on profile canvas
const profileCanvas=document.getElementById('profile');
profileCanvas.addEventListener('mousedown',function(e){
    if(_drawMode==='select'){
    ...
    const rect=profileCanvas.getBoundingClientRect();
    const mx=e.clientX-rect.left,my=e.clientY-rect.top;
    const dx=PR.dx(mx),dy=PR.dy(my);
    _dragStart={dx,dy};

    if(_drawMode==='hline'){
        _tempObj={type:'hline',x1:dx,y1:dy};
    }else if(_drawMode==='vline'){
        _tempObj={type:'vline',x1:dx,y1:dy};
    }else if(_drawMode==='trend'){
        _tempObj={type:'trend',x1:dx,y1:dy,x2:dx,y2:dy};
    }else if(_drawMode==='text'){
        const txt=document.getElementById('textInput').value.trim();
        if(txt){
            _drawnObjects.push({type:'text',x1:dx,y1:dy,text:txt,color:_colors[_drawnObjects.length%_colors.length]});
...
profileCanvas.addEventListener('mousemove',function(e){
    if(_drawMode==='select'&&_selectedObj!==null){
        const rect=profileCanvas.getBoundingClientRect();
        const mx=e.clientX-rect.left,my=e.clientY-rect.top;
        const obj=_drawnObjects[_selectedObj];
        const ddx=PR.dx(mx)-PR.dx(_dragStart.x);
        const ddy=PR.dy(my)-PR.dy(_dragStart.y);
        if(obj.type==='hline'||obj.type==='text'){
            obj.y1+=ddy;
        }else if(obj.type==='vline'){
            obj.x1+=ddx;
        }else if(obj.type==='trend'){
    ...
    if(_tempObj.type==='hline'){
        ...
        _tempObj.x2=dx;_tempObj.y2=dy;
    }
    drawUserObjects();
});

profileCanvas.addEventListener('mouseup',function(){
    if(_tempObj){
        _drawnObjects.push({..._tempObj,color:_colors[_drawnObjects.length%_colors.length]});
        _tempObj=null;
        setDrawMode('select');
    }
});
...
document.addEventListener('keydown',function(e){
    ...
    if(e.key==='Delete'||e.key==='Backspace'){
        if(_selectedObj!==null)deleteSelected();
...
function isNearObject(obj,mx,my,PR){
    const px=PR.px(obj.x1||0),py=PR.py(obj.y1||0);
    const px2=obj.x2!==undefined?PR.px(obj.x2):px;
    const py2=obj.y2!==undefined?PR.py(obj.y2):py;
    const W=PR.canvas.clientWidth,H=PR.canvas.clientHeight;
    if(obj.type==='hline'){
        return Math.abs(my-py)<8&&px>=PR.ml&&px<=W-PR.mr;
    }else if(obj.type==='vline'){
        return Math.abs(mx-px)<8&&py>=PR.mt&&py<=H-PR.mb;
    }else if(obj.type==='trend'){
        // Distance to line
        const A=mx-px,B=my-py,C=px2-px,D=py2-py;
        const len=Math.sqrt(C*C+D*D);
        if(len<1)return Math.sqrt(A*A+B*B)<8;
        const t=Math.max(0,Math.min(1,(A*C+B*D)/(len*len)));
        const dist=Math.sqrt((A-t*C)**2+(B-t*D)**2);
        return dist<8;
    }else if(obj.type==='text'){
        return mx>=px-4&&mx<=px+60&&my>=py-15&&my<=py+5;
    }
    return false;
}
...
// ===================== INIT =====================
PR.init();
document.getElementById('btnSelect').classList.add('active'); // Default active

// hide loading once data is set
const _origSetData=PR.setData.bind(PR);
PR.setData=function(d){hideLoading();_origSetData(d)};
...
class UnifiedChart(QWebEngineView):
    ...
    def set_draw_mode(self, mode: str):
        """Set drawing mode: 'select', 'hline', 'vline', 'trend', 'text'"""
        valid_modes = ['select', 'hline', 'vline', 'trend', 'text']
        if mode not in valid_modes:
            mode = 'select'
        self._js(f"setDrawMode('{mode}');")
